...
- For non-trivial changes, include a `Key Decisions` section before implementation steps that captures the highest-leverage technical choices.
- In `Key Decisions`, prefer compact entries that state the decision, main alternatives considered, chosen option, and why it was chosen.
