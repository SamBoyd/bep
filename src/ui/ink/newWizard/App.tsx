...
import { ManualProviderStep } from "./components/ManualProviderStep.js";
import { MixpanelProviderStep } from "./components/MixpanelProviderStep.js";
import {
...
type WizardPrompt = SelectPromptRequest | TextPromptRequest;
...
function isProviderStep(step: WizardStepId): boolean {
  return step.startsWith("manual_") || step.startsWith("mixpanel_");
}
...
export function InkNewWizardApp({ onComplete }: { onComplete: (result: NewWizardResult) => void }) {
  ...
  const body = (() => {
    if (isProviderStep(step) && draft.leadingIndicatorType === "manual") {
      return <ManualProviderStep prompt={prompt} selectedIndex={uiState.selectIndex} textValue={uiState.textValue} />;
    }

    if (isProviderStep(step) && draft.leadingIndicatorType === "mixpanel") {
      return <MixpanelProviderStep prompt={prompt} selectedIndex={uiState.selectIndex} textValue={uiState.textValue} />;
    }

    if ("options" in prompt) {
      return <SelectStep prompt={prompt} selectedIndex={uiState.selectIndex} />;
    }

    return <TextStep prompt={prompt} value={uiState.textValue} />;
  })();
